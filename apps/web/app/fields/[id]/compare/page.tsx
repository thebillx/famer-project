...
export default function ComparePage() {
  ...
  return <MapWorkspaceShell accountLabel={identity.data?.display_name}>{terminal ? <div className="p-8 text-center" role="alert"><h1>เซสชันหมดอายุ กรุณาเข้าสู่ระบบอีกครั้ง</h1><Link href="/login">ไปหน้าเข้าสู่ระบบ</Link></div> : field.data && history.data ? <CompareWorkspace field={field.data} observations={history.data} initialBefore={search.get("before") ?? undefined} initialAfter={search.get("after") ?? undefined} /> : <div className="p-8" role={field.isError || history.isError ? "alert" : "status"}>{field.isError || history.isError ? "โหลดข้อมูลเปรียบเทียบไม่สำเร็จ" : "กำลังเตรียมภาพเปรียบเทียบ…"}</div>}</MapWorkspaceShell>;
}
